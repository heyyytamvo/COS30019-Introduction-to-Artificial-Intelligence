...
class TruthTableGenericKB
{
    ...
    void solve(AdvancedKB kb, string query){
        ...
        if (result){
            ...
            cout << "Here is the list of model sastifying our KB" << endl;
            int count = 1;
            while (list.empty() == false){
                map<string, bool> current = list.front();
                list.pop();

                // Print all keys and values
                cout << "------" << "MODEL " << count << "------" << endl;
                for (std::map<std::string, bool>::const_iterator it = current.begin(); it != current.end(); ++it) {
