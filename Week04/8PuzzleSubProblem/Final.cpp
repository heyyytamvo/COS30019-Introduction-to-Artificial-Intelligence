...
const int FinalState[3][3] = { 
                                {0, 1, 2},
                                {3, 4, 8},
                                {8, 8, 8}  };

// despite the order of 8, find the optimal problem
    ...
    State(int _puzzle[3][3]) {
        ...
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 3; j++){
                ...
                if (_puzzle[i][j] != FinalState[i][j] && _puzzle[i][j] != 8){
                    missingTiles += 1;
                }
...
priority_queue< State, vector<State>, optionMissingTiles > pq; 
